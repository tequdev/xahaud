...
cat SetHook_test.cpp | tr '\n' '\f' | 
        ...
        do
            ...
            if [ "$WAT" -eq "0" ]
            then
                mkdir -p $CURRENT/../../../generated/hook/c
                echo '#include "api.h"' > $CURRENT/../../../generated/hook/c/test-$COUNTER-gen.c
                tr '\f' '\n' <<< $line >> $CURRENT/../../../generated/hook/c/test-$COUNTER-gen.c
                ...
                wasmcc -x c /dev/stdin -o /dev/stdout -O2 -Wl,--allow-undefined <<< "`tr '\f' '\n' <<< $line`" |
                    ...
                    xxd -p -u -c 10 | 
                    sed -E 's/../ 0x&U,/g' | sed -E 's/^/    /g' >> SetHook_wasm.h
            else
